...
function GetCoinsInArea(Area)
    ...
    for i,v in pairs(GC) do
        if v.a == Area and not string.find(v.n, "Giant") and not string.find(tostring(v.mh), "160") then
            C[#C+1] = i
        end
    end
    GC = nil
...
function GetBigChestsInArea(Area)
    ...
    local GC = Invoke("Get Coins")
    for i,v in pairs(GC) do
        if v.a == Area and not string.find(v.n, "Giant") and string.find(tostring(v.mh), "160") then
